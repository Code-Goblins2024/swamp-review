 ...
 */
export const addReview = async (content, housing_id, uuid, tag_ids, ratings) => {
  if (ratings.length == 0) {
    throw new Error("Ratings should not be empty")
  }

  const { data, error } = await supabase.from("reviews").insert(
    { content, housing_id, user_id: uuid }
  ).select()
  if (error) {
  ...
  if (tag_ids.length != 0) {
    tag_ids.forEach(async (tag_id) => {
      const { error } = await supabase.from("reviews_to_tags").insert(
        { review_id, tag_id }
      )
      if (error) {
  ...
  ratings.forEach(async (cr) => {
    const { error } = await supabase.from("reviews_to_categories").insert(
      { review_id, category_id: cr.id, rating_value: cr.rating_value }
    )
    if (error) {
